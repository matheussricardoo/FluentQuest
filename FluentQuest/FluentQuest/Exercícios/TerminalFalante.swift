...
func readUserInput() -> String {
    print("\n\(blank)\(yellow)📝 Digite sua resposta: \(reset)", terminator: "")
...
func startGame(idioma:String) {
    var exitToMenu = true // Controle para voltar ao menu
    while exitToMenu == true{ // Enquanto um idioma for selecionado, o jogo continua
        let language = idiomaSelecionado
        let voice = voices[language!] ?? "Luciana" // Define a voz para o idioma escolhido
        
        printTerminalFalante()

        print("\n\(blank)🎤 \(blue)Idioma escolhido: \(language!.uppercased()) \(flags[language!] ?? "")\(reset)")
        print("\n\(blank)\(blue)➡️  Digite '\(yellow)1\(blue)' para ouvir de novo ou '\(yellow)0\(blue)' para voltar para o menu.\(reset)")
        
        
        while exitToMenu == true {
            let (foreignWord, translation) = words[language!]?.randomElement() ?? ("", "") // Escolhe uma palavra aleatória
            
            print("\n\(blank)🔊 \(blue)Tente traduzir a palavra falada: \(reset)")
...
//                } else
                if response == "0" {
                    print("\n\(blank)\(blue)🔙 Retornando ao menu...\(reset)")
                    speak("Retornando ao menu.", voices["Português"]!)
                    exitToMenu = false // Sinaliza que o usuário quer voltar ao menu
                    break
                } else if response == "1" {
                    print("\n\(blank)\(yellow)🔄 Repetindo palavra...🔄\(reset)")
                    speak(foreignWord, voice)
                } else if response == translation {
                    print("\n\(blank)\(green)✅ Correto! A palavra '\(foreignWord)' significa '\(translation)'. 🎉\(reset)")
                    ...
                    print("\n\(blank)\(red)❌ Errado! A resposta correta era '\(translation)'. Vamos tentar outra! \(reset)")
...
func iniciarTerminalFalante(idioma:String) {
